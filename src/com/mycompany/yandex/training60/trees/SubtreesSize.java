...
public class SubtreesSize {
    static BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
    static PrintWriter out = new PrintWriter(System.out);
    static StringTokenizer tok = new StringTokenizer("");

    public static void main(String[] args) {
        new SubtreesSize().run();
        out.close();
    }
    ...
    void run() {
        ...
        dfs(vertexCnt, treeAdj, 1, 0);
        // print
        StringBuilder sb = new StringBuilder();
        for(int i = 1; i <= N; i++) {
            if (i > 1) sb.append(' ');
            sb.append(vertexCnt[i]);
        }
        out.println(sb);
    }

    private void dfs(int[] vertexCnt, List<List<Integer>> adj,
        ...
        for (int i = 0; i < adjCnt; i++) {
            ...
            if (childNode != parentNode) {
                dfs(vertexCnt, adj, childNode, fromNode);
