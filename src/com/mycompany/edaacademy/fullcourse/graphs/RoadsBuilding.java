...
public class RoadsBuilding {
    ...
    private void solve() {
        ...
        int cnt = 0;
        List<Integer> disconnectedEdges = new ArrayList<>();
        for(int v = 1; v <= n; v++) {
            if (!visited[v]) {
                dfs(v, adj, visited);
                cnt++;
                disconnectedEdges.add(v);
            }
        }
        if (cnt == 1) {
            out.println("0");
            ...
            for(int i = 0; i < cnt-1; i++) {
                sb.append(disconnectedEdges.get(i));
                sb.append(' ');
                sb.append(disconnectedEdges.get(i+1));
                sb.append('\n');
            }
            out.println(sb);
        }
